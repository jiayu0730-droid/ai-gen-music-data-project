...
* `data/platform_data_sources_step1.csv`
* `data/platform_evidence_summary.csv`
...
* `data/youtube_market_queries.csv`
* `data/youtube_public_video_metrics.csv`
* `data/youtube_public_market_summary.csv`
...
第四步，我写了自动 report generator，把 YouTube 的分析结果整理成 markdown report。
...
* `reports/youtube_public_market_analysis.md`
...
YouTube 数据显示，播放量最高的方向主要集中在 lyrical pop，尤其是：
...
当前 YouTube 数据说明，本地语言内容在多个市场都有明显需求，例如：
...
但是 jazz、lofi、bossa nova 等类型虽然总播放量不如 pop，但它们更适合作为 niche opportunity，尤其是在：

* Japanese jazz instrumental
* bossa nova instrumental
* lofi study music
* café / relaxing / work background music

这些方向更适合做差异化测试，而不是第一阶段的大众爆款方向。
...
相反，一些市场虽然 YouTube demand 没有最高，但 iTunes catalog supply 较低，可能更像潜在 market gap，例如：

* New Zealand pop lyrical
* Canada lofi instrumental
* 部分小语种 / 小众 instrumental 方向

这说明后续不能只看播放量，也要同时看供给和竞争。
...
如果后续可以拿到 distributor report，可以加入：

* Spotify streams / revenue
* YouTube Music country-level revenue
* Facebook / Instagram revenue
* Luna revenue
* 腾讯 / 网易播放量和收益单价

这样可以把当前的 market demand + supply 分析升级成完整的 business opportunity analysis。
